...
class ProfissionalService {
  ...
  private resolveWorkingConfig(data: ICreateProfissionaleDTO | IUpdateProfissionalDTO) {
    return {
      ...
      horario_inicio: this.parseTime(
        data.horario_inicio ?? data.disponibilidade_inicio,
        DEFAULT_WORKING_START,
      ),
      horario_fim: this.parseTime(
        data.horario_fim ?? data.disponibilidade_fim,
        DEFAULT_WORKING_END,
      ),
  ...
  async create(data: ICreateProfissionaleDTO) {
    ...
    if (workingConfig.horario_inicio >= workingConfig.horario_fim)
      throw badRequest("Horário inicial deve ser anterior ao final");
    if (
      (workingConfig.almoco_inicio && !workingConfig.almoco_fim) ||
      (!workingConfig.almoco_inicio && workingConfig.almoco_fim)
    )
      throw badRequest("Intervalo de almoço incompleto");
  ...
  async update(id: string, data: IUpdateProfissionalDTO) {
    ...
    if (data.email !== undefined) {
      const email = data.email.trim().toLowerCase();
      assertEmail(email);
      if (email === env("ADMIN_EMAIL").toLowerCase() || (await Cliente.exists({ email })))
    ...
    if (data.dias_trabalho !== undefined)
      payload.dias_trabalho = this.parseDays(data.dias_trabalho);
    if (data.horario_inicio !== undefined)
      payload.horario_inicio = this.parseTime(data.horario_inicio, DEFAULT_WORKING_START);
    ...
    if (data.almoco_inicio !== undefined)
      payload.almoco_inicio = this.parseTime(data.almoco_inicio, "");
    if (data.almoco_fim !== undefined) payload.almoco_fim = this.parseTime(data.almoco_fim, "");
    if (data.disponibilidade_inicio !== undefined)
      payload.disponibilidade_inicio = this.parseDate(
        data.disponibilidade_inicio,
        "Disponibilidade inicial",
      );
    if (data.disponibilidade_fim !== undefined)
      payload.disponibilidade_fim = this.parseDate(
