...
class ProfissionalService {
  private parseDate(value: Date | string, field: string): Date {
    ...
    if (Number.isNaN(date.getTime())) {
      throw badRequest(`${field} inválida`);
    }
  ...
  async create(data: ICreateProfissionaleDTO) {
    ...
    const config = this.workingConfig(data);
  ...
  async update(id: string, data: IUpdateProfissionalDTO) {
    ...
    if (data.especialidade !== undefined) {
      payload.especialidade = data.especialidade.trim();
    }
    ...
    if (data.horario_inicio !== undefined) {
      payload.horario_inicio = this.parseTime(data.horario_inicio);
    }
